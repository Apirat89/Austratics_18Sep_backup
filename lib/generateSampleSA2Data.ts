...
export interface SampleSA2Record {
  'SA2 ID': string;
  'SA2 Name': string;
  Description?: string;
  Category?: string;
 ...
 */
function generateSuburbName(): string {
 ...
 */
function generateSA2Id(stateCode: string, index: number): string {
 ...
 */
export function generateDemographicsData(count: number = 50): SampleSA2Record[] {
  const records: SampleSA2Record[] = [];
  const descriptions = [
 ...
 */
export function generateDSSData(count: number = 50): SampleSA2Record[] {
  const records: SampleSA2Record[] = [];
 ...
 */
export function generateEconomicsData(count: number = 50): SampleSA2Record[] {
  const records: SampleSA2Record[] = [];
  ...
  for (let i = 0; i < count; i++) {
    ...
    parentDescriptions.forEach(parent => {
      descriptions[parent as keyof typeof descriptions].forEach(desc => {
        const sa2Id = generateSA2Id(stateCode, i + 1);
        const sa2Name = generateSuburbName();
 ...
 */
export function generateHealthStatsData(count: number = 50): SampleSA2Record[] {
  const records: SampleSA2Record[] = [];
 ...
 */
export async function writeSampleDataFiles(outputDir: string = 'data/sa2', regionCount: number = 50) {
  ...
  try {
    ...
    await fs.writeFile(
      path.join(outputDir, 'Demographics_2023_expanded.json'),
    ...
    await fs.writeFile(
      path.join(outputDir, 'DSS_Cleaned_2024_expanded.json'),
      JSON.stringify(dssData, null, 2),
      'utf-8'
    );
    ...
    await fs.writeFile(
      path.join(outputDir, 'econ_stats_expanded.json'),
    ...
    await fs.writeFile(
      path.join(outputDir, 'health_stats_expanded.json'),
