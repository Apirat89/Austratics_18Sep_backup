...
export async function GET(request: NextRequest) {
  try {
    ...
    // Rate limiting
    const rateLimitKey = `admin-feature-stats:${authResult.user!.id}`;
    ...
    // Execute the feature stats query
    const { data, error } = await supabase.rpc('get_feature_usage_stats', {
      window_days: intervalString
    });

    if (error) {
      console.error('Error executing feature stats query:', error);
      return NextResponse.json(
    ...
    console.error('Admin feature stats API error:', error);
    return NextResponse.json(
