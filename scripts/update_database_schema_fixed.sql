-- 🚨 CRITICAL: Database Schema Update for Citation Validation (FIXED VERSION)
...
-- Step 2: Drop existing functions first to avoid signature conflicts
DROP FUNCTION IF EXISTS match_documents(vector, double precision, integer);
DROP FUNCTION IF EXISTS match_documents(vector, float, integer);
DROP FUNCTION IF EXISTS match_documents_by_type(vector, text, double precision, integer);
DROP FUNCTION IF EXISTS match_documents_by_type(vector, text, float, integer);

-- Step 3: Create the updated match_documents function with actual_pdf_pages
...
-- Step 4: Create the updated match_documents_by_type function (FIXED - renamed parameter)
CREATE FUNCTION match_documents_by_type (
  query_embedding vector(768),
...
BEGIN
  ...
  FROM document_chunks
  WHERE 1 - (document_chunks.embedding <=> query_embedding) > match_threshold
    AND document_chunks.document_type = filter_document_type
  ORDER BY document_chunks.embedding <=> query_embedding
  LIMIT match_count;
END;
$$;

-- Step 5: Create an index on actual_pdf_pages for performance
CREATE INDEX IF NOT EXISTS idx_document_chunks_actual_pdf_pages 
ON document_chunks(actual_pdf_pages);

-- Step 6: Add a validation function to check for phantom page numbers
...
-- Step 7: Create a view to show citation validation statistics
...
-- Step 8: Add helper function for getting distinct document names
...
-- These may be needed depending on your Supabase setup
...
BEGIN
  ...
  RAISE NOTICE '   1. Run the fix_existing_citations.ts script';
  RAISE NOTICE '   2. Test with problematic queries';
  RAISE NOTICE '   3. Monitor for phantom page numbers';
END $$; 